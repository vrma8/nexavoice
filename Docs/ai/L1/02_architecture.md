...
Customer voice UI (VoiceAgentCall + ConversationComponent)
  ...
  -> RTC join/publish mic (labelled Mute/Unmute control)
  -> RTM subscribe + AgoraVoiceAI events
  -> PATCH /api/conversations/:id      (mirror transcript + agent state)
  -> POST /api/stop-conversation       (customer ends; closes for the human side too)
  -> post-handover: Web Speech captions the customer mic -> POST /messages role=user
...
Backend (lib/support + lib/shop)
  ...
  -> medical guard (lib/support/medical-guard.ts): refuse every medical request;
     from the second one auto-escalate with a handoff naming the reason
  -> escalate_to_human -> SupportCase + handoff summary (live orders + cart + transcript + tracked intent) -> SSE to dashboard
...
Human dashboard (SupportDashboard, CaseWorkspace, HumanVoiceBridge)
  ...
  -> POST /api/conversations/:id/messages role=human_agent (chat + live captions)
  -> POST /api/cases/:id/resolve / :leave (ends for the customer too)
```

## Conversation State Machine

`AI_HANDLING -> WAITING_FOR_HUMAN -> HUMAN_HANDLING -> RESOLVED`, with `CLOSED` when either side ends the conversation (`closeConversation`, `resolveCase` or `humanLeaveCase` — a call is two-sided, so ending on one side ends the other). Transitions happen only in `lib/support/store.ts`; clients poll or subscribe.
...
- Vercel deployment as a single Next.js app with server env vars. Each route is an
  independent function instance, so the support store mirrors to PostgreSQL
  (`lib/support/persist.ts` → one `StoreState` JSONB row) when `DATABASE_URL` exists,
  otherwise memory-only. Shop data is always plain Prisma rows.
...
- Conversation/case state crosses function instances through the durable mirror; a write is
  flushed inside the request that made it (never from a timer). Shop state needs no mirror —
  it is read from and written to PostgreSQL directly.
