...
- `escalation/request/route.ts`: escalation endpoint used by the AI tool layer (there is no customer-facing connect-to-human button).
- `cases/`, `cases/[id]/`, `cases/[id]/{accept,takeover,resolve,leave}/`: human agent case lifecycle (accept issues the human's channel token; takeover makes the AI speak + leave; leave/ resolve end the call for the customer too).
...
- `ConversationComponent.tsx`: RTC join, mic publish (labelled Mute/Unmute), toolkit init, transcript/metrics/issues state, backend sync (`useConversationSync.ts`), human-join detection, two-sided hang-up (polls for the human resolving/leaving), post-handover customer captions; `HandoffBanner.tsx` shows escalation/takeover status.
...
- `ClientChat.tsx`: customer chat (create conversation bound to the signed-in client, send, heartbeat, poll for human replies, close on leave; ask for a human in the chat, Nexa escalates).
- `SupportDashboard.tsx`: human dashboard (SSE + poll, live calls/chats, queue, handoff summary, customer details, accept).
- `CaseWorkspace.tsx` + `HumanVoiceBridge.tsx`: case page (full AI + customer + human transcript, join call as uid `654321` → takeover, chat reply, resolve/leave; `HumanVoiceBridge` captions the human's speech via Web Speech into `human_agent` messages).
...
- `agent-config.ts` / `agent-prompt.ts` / `agent-tools.ts`: agent pipeline, NexaMart system prompt + language-aware greetings (the greeting confirms the customer's saved `preferredLanguage`), tool schemas + Agora inline REST tool wiring. The prompt is intent-first (detect → act with tools → escalate with a complete summary) and has a degraded, tool-free variant for sessions whose tools could not be wired (`toolsAvailable: false`) — a tool-less agent must never be told it has tools, or it invents products and order data.
...
- `support/types.ts`, `support/store.ts`, `support/tools.ts`, `support/medical-guard.ts`: conversation/case model, in-memory store + event bus + dashboard snapshot, guarded tool execution (intent tracking, `DATABASE_UNAVAILABLE` classification) + handoff summary (live orders + cart + transcript), and the medical-safety policy (refuse first, auto-escalate on the second request).
...
- `api.ts`: browser API client for the shop, chat, conversations, escalation, dashboard, cases (incl. `leaveCase`).
- `hooks/use-speech-recognition.ts`: Web Speech captions used by both sides after handover so the case transcript contains what the human (and customer) said, not just the AI.
...
- Change medical policy (refusal copy, escalation threshold, medical products) -> `lib/support/medical-guard.ts` + `lib/support/tools.ts` + `lib/agent-prompt.ts`.
- Change post-handover captions -> `hooks/use-speech-recognition.ts`, `components/HumanVoiceBridge.tsx`, `components/ConversationComponent.tsx`.
