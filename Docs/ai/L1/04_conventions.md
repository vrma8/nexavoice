...
- `hooks/use-speech-recognition.ts` owns the Web Speech recognizer: init `onFinal` via an effect (never in render), auto-restart, and never call `stop()` manually as a side effect.
- Post-handover captions go through `POST /api/conversations/:id/messages` (role `user` on the customer, `human_agent` on the human side) — never through `PATCH transcript` (that mirrors the toolkit and would duplicate/rewrite AI turns).
