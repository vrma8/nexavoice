...
- **Anything added to `lib/support/types.ts` must be mirrored** in
  ...
  vanishes on a serverless deployment. New `Conversation`/`SupportCase` fields (e.g. `endedBy`, `humanLeftAt`, `medicalRequestCount`) are carried by the whole object — no separate snapshot field needed.
- **Medical safety is enforced in the tool layer, not only in the prompt.** `lib/support/medical-guard.ts` + `executeTool` refuse medical searches, filter the `Medicine` category out of results, block write tools that would add a medical product, and auto-escalate on the second request. Never bypass it by exposing medicine products to the LLM or relaxing the threshold.
- **The call is two-sided.** Customer close → `closeConversation` marks `customerLeftAt` and the human side leaves; human resolve/leave → `resolveCase`/`humanLeaveCase` closes the conversation (`endedBy: 'human'`) and the customer's UI polls `ended` and hangs up. Do not delete conversations in `resolveCase` — the case page keeps the full transcript until `pruneSnapshot` TTLs it.
...
- **The toolkit only transcribes customer + AI while the AI pipeline is running.** After handover, both sides caption their own mic via `hooks/use-speech-recognition.ts` (Web Speech API) into `POST /api/conversations/:id/messages`; Firefox does not support it (`captionsSupported: false` — the call works, captions are skipped). Web Speech emits the same final result repeatedly — dedupe by text + timestamp in the consumer.
- **Post-handover captions are stored with `role: user` / `human_agent`, never as AI turns** — the messages route only runs `runChatTurn` while the conversation is `AI_HANDLING`, so captions after a takeover are safe.
...
| Agent talks but never looks up orders | server log `[invite-agent] Backend tools disabled` → `AGENT_TOOLS_BASE_URL`/`AGENT_TOOLS_SECRET`; `lib/agent-tools.ts`. The call UI also shows an amber "can't reach your cart or orders" banner and the agent runs the degraded, tool-free prompt (`toolsAvailable: false` in `lib/agent-prompt.ts`) |
| Agent invents products / quotes made-up prices | tools disabled (see above) or the order database is down — tools then return `DATABASE_UNAVAILABLE` with a never-invent instruction; check `/api/health` → `database`; `lib/db.ts` |
| Every cart/order tool fails | `/api/health` → `database` names the exact error; Supabase free-tier projects pause after inactivity (restore in the dashboard); `lib/db.ts` |
