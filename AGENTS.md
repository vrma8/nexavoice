...
- Default agent config (`lib/agent-config.ts`): Agora-managed Deepgram STT (`multi`), OpenAI `gpt-4o-mini`, MiniMax TTS; NexaMart system prompt in `lib/agent-prompt.ts`; inline REST tools from `lib/agent-tools.ts`. `/api/health` reports the deployment self-check (incl. a live `database` probe). `.env.local` needs only Agora credentials — `AGENT_TOOLS_BASE_URL`/`AGENT_TOOLS_SECRET` are optional now (tool origin comes from the request URL, the secret is derived from the App Certificate).
- The system prompt (`lib/agent-prompt.ts`) is intent-first: detect what the customer wants → resolve it with tools → escalate only when needed, with a complete handoff summary. When a session has **no** tools (`toolsAvailable: false`, e.g. a deployment the engine cannot reach back into) the SAME builder returns a degraded prompt that forbids pretending and inventing data instead of describing tools the model cannot call — a tool-less agent that believes it has tools happily "checks" a cart it cannot see and makes up products. The voice UI shows an amber banner for the same condition (`tools_enabled: false` from `/api/invite-agent`).
...
- Database outages are surfaced to the model as `DATABASE_UNAVAILABLE` (classified by `isDatabaseUnavailableError()` in `lib/db.ts`) with an explicit never-invent instruction — never swallow them into a generic failure the LLM will improvise around. Every tool call also records the intent it reveals on `conversation.context.intent` (`TOOL_INTENT`), which is the fallback intent in the handoff summary.
- Medical safety is enforced in `lib/support/medical-guard.ts` + `executeTool`, not only in the prompt: medical searches are refused and medicine-category products are filtered from results; write tools that would add a medical product are refused; the second medical request (`medicalRequestCount`) auto-creates a case with `intent: 'medical_advice'`. Do not weaken or bypass this layer.
- The voice call is two-sided: `endConversation()`/`closeConversation` (customer) and `/api/cases/:id/leave` or `resolveCase` (human) each end the OTHER side too (`endedBy` / `customerLeftAt` / `humanLeftAt`). `resolveCase` must NOT delete the conversation — the case page keeps the full AI + customer + human transcript until `pruneSnapshot` TTLs it.
...
- `lib/agent-prompt.ts`: NexaMart system prompt (voice + chat variants; intent-first, with a degraded tool-free variant when `toolsAvailable: false`), language-aware greetings that confirm the saved preference, `normalizeLanguageName`, failure message.
...
- `components/ConversationComponent.tsx`: RTC join, mic publication, `AgoraVoiceAI` init, transcript state, renewals, backend sync (`useConversationSync.ts`), human-join detection (uid `654321`), labelled Mute/Unmute controls, two-sided hang-up (polling sees the human resolve/leave and closes the call), post-handover customer captions.
- `components/ClientChat.tsx`, `components/SupportDashboard.tsx`, `components/CaseWorkspace.tsx`, `components/HumanVoiceBridge.tsx`, `components/HandoffBanner.tsx`: chat, dashboard, case page, voice takeover (with human-side Web Speech captions), caller-side handoff status.
- `hooks/use-speech-recognition.ts`: browser captions used after handover so the case transcript contains what the human said, not just the AI.
- `lib/support/medical-guard.ts`: shared medical-safety policy — Nexa never gives medical advice or recommends medical products; first request is refused, second auto-escalates (`MEDICAL_ESCALATION_LIMIT`) with a handoff summary naming the reason.
...
pnpm run verify:api
pnpm run build
pnpm run test:voice-tools    # E2E over the engine's REST tool path (needs dev server + db)
pnpm run test:db-unavailable # DATABASE_UNAVAILABLE classification with an unreachable db
```
