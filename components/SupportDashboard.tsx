          ...
          <>
            ...
            <div className="grid flex-1 grid-cols-1 gap-4 overflow-y-auto p-4 lg:grid-cols-2">
              <section className="space-y-3">
                ...
                {(selectedCase.handoff.cart?.items?.length ?? 0) > 0 && (
                  <ListField
                    label={`Cart at handoff (₹${selectedCase.handoff.cart!.total_inr.toLocaleString('en-IN')})`}
                    items={selectedCase.handoff.cart!.items}
                  />
                )}
              </section>
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-[11px] text-zinc-500">
        ...
        {c.humanLeftAt && c.status !== "RESOLVED" && <span className="ml-1 text-amber-400">· agent left</span>}
      </p>
    </button>
  );
}
