...
export default function CaseWorkspace({ caseId }: { caseId: string }) {
  ...
  /**
   * The customer ended the chat/call (closed the dock, closed the tab) while
   * this case was open — the conversation is CLOSED and the case is flagged.
   * The human side therefore ends too: no join button, no reply box.
   */
  const customerEnded =
    conversation?.state === 'CLOSED' ||
    Boolean(supportCase?.customerLeftAt) ||
    conversation?.endedBy === 'customer';
  /** The human agent already left this call (case stays for follow-up). */
  const humanLeft = Boolean(supportCase?.humanLeftAt);
  ...
  /**
   * Leaving the voice call ends it for the customer too (two-sided connection):
   * the backend closes the conversation, the customer's call UI polls it and
   * hangs up. Skipped when the customer already ended it or it is resolved.
   */
  const handleLeaveCall = async () => {
    const canEnd = supportCase && ['WAITING_FOR_HUMAN', 'HUMAN_HANDLING'].includes(supportCase.status);
    if (canEnd && !customerEnded) {
      try {
        const result = await leaveCase(caseId);
        setDetail((prev) =>
          prev
            ? { ...prev, case: result.case, conversation: result.conversation, now: Date.now() }
            : prev,
        );
      } catch (err) {
  ...
  // Two-sided call: if the customer ends the call while we are in it, leave the
  // RTC channel automatically instead of talking to an empty channel.
  useEffect(() => {
    if (customerEnded && voice) {
      setVoice(null);
      setVoiceJoined(false);
      setTakeover('idle');
    }
    ...
    <div className="flex h-[calc(100vh-80px)] gap-4 text-zinc-100">
      ...
      <aside className="flex w-[34%] min-w-[320px] flex-col gap-4 overflow-y-auto">
        <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-4">
          ...
          {isVoice && isOpen && supportCase.status === 'HUMAN_HANDLING' && !voice && !customerEnded && !humanLeft && (
          ...
          {(customerEnded || humanLeft) && isOpen && (
            <p className="mt-3 rounded-md border border-amber-800 bg-amber-950/30 p-2 text-xs text-amber-200">
              {customerEnded
                ? 'The customer ended this conversation. The call/chat has ended on your side too — mark resolved or leave the case for callback.'
                : 'You left this call. The conversation was ended for the customer as well; mark resolved if the issue is handled, or the case stays for follow-up.'}
            </p>
          )}
          {voice && (
            <div className="mt-3 space-y-2">
              <AgoraProvider>
                <HumanVoiceBridge
                  ...
                  language={supportCase.handoff.language}
        ...
        {/* What was sitting in their cart when the AI handed the case over. */}
        {(supportCase.handoff.cart?.items?.length ?? 0) > 0 && (
          <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-4 text-sm">
            <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-zinc-500">
              Their cart (not yet ordered)
            </h3>
            <ul className="space-y-1">
              {supportCase.handoff.cart!.items.map((item, i) => (
                <li key={i} className="text-xs text-zinc-300">{item}</li>
              ))}
            </ul>
            <p className="mt-2 text-xs text-zinc-400">
              Cart total ₹{supportCase.handoff.cart!.total_inr.toLocaleString('en-IN')}
            </p>
          </div>
        )}
      </aside>
